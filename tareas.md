# Proyecto Agraria

## Tareas para realizar

### Secciones

#### General

- [x] investigar por que cuando la computadora esta activo la red de internet pero no hay internet tarda mas en cargar los datos de MS ACCESS
- [ ] verificar que carguen bien los controles cuando se usan Tareas Largas
- [ ] mejorar mensajes de errores depende de la excepcion ocurrida
- [ ] verificar que todos los data grid view solo se seleccionen de a una fila

#### Proveedores

#### Actividades

- [x] implementar servicios y repositorios

#### Inventario

- [x] implementar servicios y repositorios

#### hoja de vida

- [x] mostrar cartel de confirmacion de proceso finalizado al guardar hojadevida
- [ ] cambiar la columna de nombre por la de numero
- [ ] al cargar la pantalla de ingreso de hoja de vida que cargue el ultimo número +1 como sugerencia

#### Usuarios

- [x] agregar al modulo de gestion el atributo activo

- [x] eliminar el boton de eliminar

#### Venta

- [x] Error al confirmar la venta

- [x] Error al buscar 

#### paniol

- [x] preguntar si dejamos el boton de eliminar herramienta

#### Entorno formativo

- [x] revisar por que no carga el modulo de entorno formativo, seguramente son las bases de datos
- [x] al modificar entorno formativo bloquear el boton si no esta todo ok
- [x] en tipo y entornos, verificar accion de botones

#### productos

- [ ] agregar atributo de activo

#### Produccion

- [x] agregar descripcion al insertar venta

- [ ] agregar mas filtros de busqueda

- [x] ver en el pdf cuando son dos paginas por que la imagen se pasa a la otra pagina

- [ ] agregar boton imprimir directo

#### Reportes

- [x] btnActividades: crear un metodo nuevo en el repositorio con un sql y crear un record para obtener los nobres de las areas y entornos en ves de mostrar el id

- [x] btnUsuarios: crear un metodo nuevo en el repositorio con un sql y crear un record para obtener los nombres del tipo de usuario en ves de mostrar el id

- [x] btnEntornos: crear un metodo nuevo en el repositorio con un sql y crear un record para obtener el nombre de los tipos de entornos(areas)

- [x] btnEntornoFormativo: crear un metodo nuevo en el repositorio con un sql y crear un record para obtener los nombres y apellidos de los usuarios y el nombre del entorno al que pertenecen en ves de los id.

- [ ] agregar boton para exportar a PDF. crear html para cada caso
