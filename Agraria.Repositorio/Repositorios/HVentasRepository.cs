    ...
    public class HVentasRepository : BaseRepositorio, IHVentasRepository
    {
        public Result<HVentas> Add(HVentas venta)
        {
            try
            {
                ...
                using var cmd = new SqlCommand("INSERT INTO H_Ventas (Cod_Usuario,  Subtotal, Descu, Total, Fecha_Hora) VALUES (@Cod_Usuario, @Subtotal, @Descu, @Total, @Fecha_Hora)", conexion);
                cmd.Parameters.AddWithValue("@Cod_Usuario", venta.Cod_Usuario);
                cmd.Parameters.Add("@Subtotal", SqlDbType.Decimal).Value = venta.Subtotal;
                cmd.Parameters.Add("@Descu", SqlDbType.Decimal).Value = venta.Descu;
        ...
        public Result<HVentas> GetById(int id)
        {
            try
            {
                ...
                {
                    conexion.Open();
                    using var cmd = new SqlCommand("SELECT Id_Remito,Cod_Usuario,Fecha_Hora,Subtotal,Descu,Total FROM H_Ventas WHERE Id_Remito = @id", conexion);
        ...
        public Result<HVentas> Update(HVentas venta)
        {
            ...
            {
                using var conexion = Conexion();
                using var cmd = new SqlCommand("UPDATE H_Ventas SET Cod_Usuario = @Cod_Usuario, Subtotal = @Subtotal, Descu = @Descu, Total = @Total, Fecha_Hora = @Fecha_Hora WHERE Id_Remito = @Id_Remito", conexion);
